...
func TestGetSensorEventsHistory(t *testing.T) {
	t.Run("GET_sensor_history", func(t *testing.T) {
		validDate := "start_date=2005-12-12T12:40:56&end_date=2006-12-12T12:40:56"
		reverseDate := "start_date=2006-12-12T12:40:56&end_date=2005-12-12T12:40:56"
		...
		table := []struct {
			...
			{"ok", "/1/history?" + validDate, http.StatusOK},
			{"bad_start_and_time", "/1/history?start_date=10&end_date=20", http.StatusBadRequest},
			{"bad_sensor_id", "/a/history?" + validDate, http.StatusUnprocessableEntity},
			{"sensor_not_found", "/0/history?" + validDate, http.StatusNotFound},
			{"right_format_but_incorrect_order", "/1/history?" + reverseDate, http.StatusBadRequest},
		}

		for _, tt := range table {
			t.Run(tt.name, func(t *testing.T) {
				w := httptest.NewRecorder()
				req, _ := http.NewRequest(http.MethodGet, "/sensors"+tt.input, nil)
				req.Header.Add("Accept", "application/json")
				router.ServeHTTP(w, req)

				assert.Equal(t, tt.want, w.Code, "Получили не тот код")
			})
		}
	})
}
